...
class BaiduVoiceToTxt():
    ...
    def change_file_format(self,filepath):
        ...
        print(change_file_format_command)
    ...
    def transfer_voice_to_srt(self,access_token,filepath):
        ...
        #1536 普通话将post的dev_pid参数修改为1536,英文1737
        post_data = {
            "format": "pcm",
            "rate": 16000,
            "dev_pid": 1536,
...
if __name__ == "__main__":
    ...
    for all_item in all_path:
        video_dir = os.path.join(parent_dir, all_item)
        print(video_dir)
        # 自己要进行音文转换的音视存放的文件夹
        #video_dir = ".\\video\\"
        all_video_file =[]
        ...
        for video_file_name in all_video_file:
            ...
            video_file_srt_path = f"{video_dir}\\{video_file_name[:-4]}.srt"
            # 以覆盖形式打开.txt文件
            video_file_srt_obj = open(video_file_srt_path,'w+',encoding='utf-8')
            ...
            for filename in all_pcm_file:
                j += 1
                print(f"all_pcm_file,filename:{filename}")
                ...
                if (os.path.isfile(filepath)):
                    ...
                    print(f"baidu transfer_voice_to_srt:"+response_text)
                    # 以json形式读取返回结果
                    if len(response_text) > 0:
                        ...
                        video_file_srt_obj.writelines(f"{j}\r\n")
                        video_file_srt_obj.writelines(f"{time_start_str} --> {time_end_str}\r\n")
                        print(f"{time_start_str}-{time_end_str}({j}/{pcm_file_num})转换过程获取token错误")
                        video_file_srt_obj.writelines(f"{subtitle_format}转换过程获取token错误\r\n")
                        video_file_srt_obj.writelines(f"\r\n")
